...
boolean MLX90632::begin(uint8_t deviceAddress, TwoWire &wirePort, status &returnError)
{
  ...
  while (eepromBusy())
  {
    ...
    {
      if (_printDebug)
        _debugPort->println(F("EEPROM busy timeout"));
...
float MLX90632::getObjectTemp(status &returnError)
{
  ...
  for (uint8_t i = 0; i < 3; i++)
  {
    ...
    double S = ((double)(lowerRAM * 1.0) + (double)(upperRAM * 1.0)) / 2.0;
...
{
  if (getStatus() & ((uint16_t)1 << BIT_DEVICE_BUSY))
    return (true);
  return (false);
}
...
{
  if (getStatus() & ((uint16_t)1 << BIT_EEPROM_BUSY))
    return (true);
  return (false);
}
